...
public class DriveCommands {
     ...
     */
    public static Command joystickApproach(CommandSwerveDrivetrain drive, DoubleSupplier ySupplier,
        ...
        return Commands.run(
                () -> { // Command
                    ...
                    Translation2d offsetVector = new Translation2d(alignController.calculate(distanceToGoal), 0)
                            .rotateBy(robotToGoal.getAngle());

                    // Calculate total linear velocity
                    Translation2d linearVelocity = getLinearVelocityFromJoystick(
                            0,
                            ySupplier.getAsDouble()).rotateBy(approachSupplier.get().getRotation()).plus(offsetVector);

                    double omega = angleController.calculate(drive.getState().Pose.getRotation().getRadians(),
                            approachSupplier.get().getRotation().rotateBy(Rotation2d.k180deg).getRadians());
    ...
    public static Command autoApproach(CommandSwerveDrivetrain drive, DoubleSupplier speed,
            Supplier<Pose2d> approachSupplier) {
