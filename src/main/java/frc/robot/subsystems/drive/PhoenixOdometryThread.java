...
public class PhoenixOdometryThread extends Thread {
  ...
  public Queue<Double> registerSignal(
      StatusSignal<Angle> signal, StatusSignal<AngularVelocity> derivative) {
    ...
    try {
      ...
      newSignals[phoenixSignals.length + 1] = derivative;
  ...
  public void run() {
    while (true) {
      ...
      try {
        ...
        {
          int i = 0;
          for (var queue : phoenixQueues) {
            queue.offer(
                BaseStatusSignal.getLatencyCompensatedValueAsDouble(
                    phoenixSignals[i++], phoenixSignals[i++]));
          }
        }
        for (int i = 0; i < genericSignals.size(); i++) {
          genericQueues.get(i).offer(genericSignals.get(i).getAsDouble());
        }
        for (var queue : timestampQueues) {
